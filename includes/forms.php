...
function software_issue_manager_set_issue_entry($atts) {
	...
	if (!in_array('emd_iss_email', $req_hide_vars['hide'])) {
		//text
		$form->add('label', 'label_emd_iss_email', 'emd_iss_email', __('Email', 'software-issue-manager') , array(
			'class' => 'control-label'
		));
		$attrs = array(
			'class' => 'input-md form-control',
			'placeholder' => __('Email', 'software-issue-manager')
		);
		if (!empty($current_user) && !empty($current_user->user_email)) {
			$attrs['value'] = (string)$current_user->user_email;
		}
		if (!empty($_GET['emd_iss_email'])) {
			$attrs['value'] = sanitize_email($_GET['emd_iss_email']);
		} elseif (!empty($set_arrs['attr']['emd_iss_email'])) {
			$attrs['value'] = $set_arrs['attr']['emd_iss_email'];
		}
		$obj = $form->add('text', 'emd_iss_email', '', $attrs);
		$zrule = Array(
			'dependencies' => array() ,
			'email' => array(
				'error',
				__('Email: Please enter a valid email address', 'software-issue-manager')
			) ,
		);
		if (in_array('emd_iss_email', $req_hide_vars['req'])) {
			$zrule = array_merge($zrule, Array(
				'required' => array(
					'error',
					__('Email is required', 'software-issue-manager')
				)
			));
		}
		$obj->set_rule($zrule);
...
function software_issue_manager_process_issue_entry($atts) {
	...
	if ($show_form == 1) {
		...
		return emd_submit_php_form('issue_entry', 'software_issue_manager', 'emd_issue', 'publish', 'publish', $success_msg, $error_msg, 0, 1, $atts);
	} else {
