...
if (isset($_GET['software_issue_manager_optin'])) {
	if (!function_exists('wp_get_current_user')) {
		require_once (ABSPATH . 'wp-includes/pluggable.php');
	}
	if ($_GET['software_issue_manager_optin'] == 1) {
		//opt-in
	...
	wp_redirect(esc_url(remove_query_arg('software_issue_manager_optin')));
	exit;
} else {
...
function software_issue_manager_show_optin() {
	...
	if (!get_option('software_issue_manager_tracking_optin')) {
		...
		$tr_link = implode(' ', array(
			sprintf(__('%sDo not allow%s', 'software-issue-manager') , '<a href="' . admin_url('admin.php?page=software_issue_manager&software_issue_manager_optin=0') . '" class="button-secondary" id="software-issue-manager-do-not-allow-tracking">', '</a>') ,
			sprintf(__('%sAllow%s', 'software-issue-manager') , '<a href="' . admin_url('admin.php?page=software_issue_manager&software_issue_manager_optin=1') . '" class="button-primary" id="software-issue-manager-allow-tracking">', '</a>') ,
		));
		...
		echo '<li class="emd-permission"><i class="dashicons dashicons-email-alt"></i><div><span>' . __('Newsletter', 'software-issue-manager') . '</span><p>' . __('Updates, announcements, marketing, no spam', 'software-issue-manager') . ', <a href="https://emdplugins.com/subscription-preferences/" target="_blank">unsubscribe anytime</a></p></div></li>';
		echo '</ul></div></div>';
	} else {
...
function software_issue_manager_show_rateme_action() {
	...
	if (5 === $min_trigger) {
		...
		$response['redirect'] = "https://emdplugins.com/plugin_tag/sim-com";
		$min_trigger = - 1;
	}
...
function software_issue_manager_show_rateme_notice() {
	...
	if ($min_count > 5 || ($min_trigger == 5 && $installed_date <= $today)) {
		...
		$message_end1 = '<li><a data-rate-action="do-rate" data-plugin="software_issue_manager" href="#">' . __('Ok, you deserve it', 'software-issue-manager') . '</a>
       </li>
...
function software_issue_manager_deactivation_feedback_box() {
	...
	foreach ($reasons as $key => $reason) {
		...
		switch ($key) {
			...
			case 9:
				$feedback_vars['reasons'].= 'data-input-type="textarea"';
				$feedback_vars['reasons'].= 'data-input-placeholder="' . __('We are sorry to hear that. Please share your previous version number before update, new updated version number and what happened', 'software-issue-manager') . '"';
			break;
			case 5:
				$feedback_vars['reasons'].= 'data-input-type="textarea"';
				$feedback_vars['reasons'].= 'data-input-placeholder="' . __('We are sorry to hear that. Please share what happened', 'software-issue-manager') . '"';
			break;
			case 22:
				$feedback_vars['reasons'].= 'data-input-type="textarea"';
